...
class CarWaleScraper(BaseScraper):
    ...
    async def scrape_listings(self, page, session) -> List[Dict]:
        listings = []
        await page.goto("https://www.carwale.com/used-cars/", timeout=60000)
        ...
        cards = await page.query_selector_all('[class*="card"], [data-testid*="card"], a[href*="/used-car"]')
        if not cards:
            cards = await page.query_selector_all("a[href*='/used']")
    ...
    async def _extract(self, page, card) -> Optional[Dict]:
        ...
        full_url = f"https://www.carwale.com{url}" if url.startswith("/") else url
