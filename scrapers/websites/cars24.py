...
class Cars24Scraper(BaseScraper):
    ...
    async def scrape_listings(self, page, session) -> List[Dict]:
        listings = []
        await page.goto("https://www.cars24.com/buy-used-cars/", timeout=60000)
        ...
        cards = await page.query_selector_all('[data-testid="listing-card"], a[href*="/buy-used-car"]')
        if not cards:
    ...
    async def _extract(self, page, card) -> Optional[Dict]:
        title_el = await card.query_selector("h3, [class*='title'], [class*='name']")
        ...
        return {
            ...
            "fuel_type": "Petrol" if "Petrol" in detail_text else ("Diesel" if "Diesel" in detail_text else None),
            "transmission": "Automatic" if "Automatic" in detail_text else ("Manual" if "Manual" in detail_text else None),
        }
